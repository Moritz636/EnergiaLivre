...
export default function CheckoutMemberPlusPage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (!user) {
        router.push('/login?redirect=/checkout-member-plus');
        return;
      }
      setUser(user);
        ...
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400">
              Acesse o mapa de geradores da sua região e dê match para negociar energia direta.
            </p>
          </div>
          ...
          {alreadyActive && (
            <div className="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center gap-3">
              ...
              <p className="text-emerald-300 text-sm">
                Você já é Member Plus. Restam <strong>{daysRemaining} dias</strong> de acesso.
              </p>
              <Link
                href="/dashboard/match"
                className="ml-auto px-3 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 text-xs font-bold transition"
              >
                Ir para o Match →
              </Link>
            </div>
            ...
            <div className="text-center">
              ...
              <ul className="space-y-3 mb-8 text-left max-w-md mx-auto">
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-slate-300 text-sm">
                    <strong className="text-white">Mapa interativo</strong> com geradores próximos (raio ajustável)
                  </span>
                </li>
                <li className="flex items-start gap-3">
                  <div className="w-5 h-5 rounded-full bg-yellow-500/20 flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <span className="text-slate-300 text-sm">
                    <strong className="text-white">Match com 1 clique</strong>: curta perfis e envie propostas
                  </span>
                </li>
                <li className="flex items-start gap-3">
                  <div className="w-5 h-5 rounded-full bg-yellow-500/20 flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <span className="text-slate-300 text-sm">
                    <strong className="text-white">Chat integrado</strong> para negociar condições
                  </span>
                </li>
              </ul>
              ...
              >
                {alreadyActive ? (
                  ...
                  <>
                    <Sparkles className="w-5 h-5" /> Ativar Member Plus Agora
                  </>
                )}
              </button>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-8">
            {[
              { icon: ShieldCheck, title: 'Pagamento seguro', desc: 'Stripe processa 100%' },
          ...
          <p className="text-center text-[10px] text-slate-600">
            Ao assinar, você concorda com nossos Termos de Uso. Member Plus é um serviço adicional
            que não está coberto pelos planos de assinatura de energia.
          </p>
        </div>
      </div>
    </div>
